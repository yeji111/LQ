...
void plot(T hist, TString name){
    ...
    hist->Write();
    hist->DrawClone();
    c->Print(Form("%s_BG.pdf",name.Data()));
}
...
void ana_lq_BG(){
    ...
    auto df_s0_HLT = df3000.Filter("HLT_IsoMu24","SingleMuon Trigger");
    ...
    //histograms 
    auto h_muon_pt = df_s5_cJet.Define("Muon1_pt","Muon_pt[goodMuon_leading_idx]").Histo1D({"h_muon_pt", "h_muon_pt", 40, 0, 200}, "Muon_pt");
    auto h_muon_eta = df_s5_cJet.Define("Muon1_eta","Muon_eta[goodMuon_leading_idx]").Histo1D({"h_muon_eta", "h_muon_eta", 50, -5, 5}, "Muon_eta");
    
    auto h_tau_pt = df_s5_cJet.Histo1D({"h_tau_pt", "h_tau_pt", 40, 0, 200}, "Tau_pt");
    auto h_tau_eta = df_s5_cJet.Histo1D({"h_tau_eta", "h_tau_eta", 50, -5, 5}, "Tau_eta");
    ...
    df_s1_Muon.Snapshot("tree", "f_BG.root");
    TFile f("out_BG.root", "recreate");
